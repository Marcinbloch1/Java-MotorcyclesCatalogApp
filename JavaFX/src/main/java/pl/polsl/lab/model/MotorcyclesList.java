...
public class MotorcyclesList implements Serializable {
    ...
    public void getSearchedMotorcycledList(String option, String searchText) throws MyException {
        ...
        switch (option) {
            case "Category" -> findByCategory(searchText);
            case "Brand" -> findByBrand(searchText);
            case "Model" -> findByModel(searchText);
            case "ProductionYear" -> findByProductionYear(searchText);
            case "EngineCapacity" -> findByEngineCapacity(searchText);
            case "HorsePower" -> findByHorsePower(searchText);
            case "MaxSpeed" -> findByMaxSpeed(searchText);
            default -> throw new MyException("There is no such category");
        }
